...
async function writeFilesPromise(posts, config) {
    ...
    await writeMetaMarkdownFilesPromise(posts, config);
...
async function processPayloadsPromise(payloads, loadFunc) {
    ...
    if (failedCount === 0) {
        ...
        console.log(
            'Done, but with ' + chalk.red(failedCount + ' failed') + '.'
        );
    }
}
...
async function writeMetaMarkdownFilesPromise(posts, config) {
    // package up posts into payloads
    let skipCount = 0;
    let delay = 0;
    const payloads = posts.flatMap((post) => {
        const destinationPath = getPostPath(post, { ...config, isMeta: true });
        if (checkFile(destinationPath)) {
            // already exists, don't need to save again
            skipCount++;
            return [];
    ...
    if (remainingCount + skipCount === 0) {
        ...
        await processPayloadsPromise(payloads, loadMetaMarkdownFilePromise);
    }
}
...
async function loadMetaMarkdownFilePromise(post) {
    let output = '---\n';
...
function getPostPath(post, config) {
    ...
    if (config.prefixDate) {
        slugFragment = dt.toFormat('yyyy-LL-dd') + '__' + slugFragment;
    }

    // use slug fragment as folder or filename as specified
    if (config.isMeta) {
        pathSegments.push(slugFragment, 'meta.mdx');
    } else if (config.postFolders) {
        pathSegments.push(slugFragment, `${post.meta.lang || 'index'}.mdx`);
    } else {
